...
function Get-BinaryPackagePaths {
    ...
    return @(
        @("$BasePath\NetCoreEx.BinaryExtensions.nuspec", "$BasePath\..\NetCoreEx.BinaryExtensions\"),
        @("$BasePath\NetCoreEx.Geometry.nuspec", "$BasePath\..\NetCoreEx.Geometry\")
    );
}
...
function Get-SourcePackagePaths {
    ...
    param ([string]$BasePath = ".")
    return @(
        "$BasePath\NetCoreEx.Source.nuspec"
    );
}
